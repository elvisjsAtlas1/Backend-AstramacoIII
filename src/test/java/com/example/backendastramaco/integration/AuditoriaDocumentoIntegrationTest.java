...
class AuditoriaDocumentoIntegrationTest extends AuditoriaDocumentoBaseIntegrationTest {
    ...
    @Test
    @DisplayName("Debe listar todas las auditorías de documentos (200 OK)")
    void listar_DebeRetornarPaginaDeAuditorias() throws Exception {
        String token = obtenerTokenAdmin();

        mockMvc.perform(get("/api/auditoria/documentos")
    ...
    @Test
    @DisplayName("Debe listar auditorías por ID de documento (200 OK)")
    void listarPorDocumento_DebeRetornarPagina() throws Exception {
        String token = obtenerTokenAdmin();

        mockMvc.perform(get("/api/auditoria/documentos/documento/1")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", isA(java.util.List.class)));
    }
    ...
    void listarPorTransportista_DebeRetornarPagina() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/documentos/transportista/1")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", isA(java.util.List.class)));
    }
    ...
    void listarPorAccion_DebeRetornarPagina() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/documentos/accion/CREATE")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", isA(java.util.List.class)));
    }
    ...
    void listarPorRangoFechas_DebeRetornarPagina() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/documentos/fechas")
                        .param("inicio", inicio)
                        .param("fin", fin)
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
    ...
    void getResumenAuditorias_DebeRetornarMapaConConteos() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/documentos/resumen")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.UPDATE_ESTADO").exists()); // Incluye la acción extra de este controlador
    }
}