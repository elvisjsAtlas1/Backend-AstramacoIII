...
class AuditoriaUsuarioIntegrationTest extends AuditoriaUsuarioBaseIntegrationTest {
    ...
    @Test
    @DisplayName("Debe listar todas las auditorías de usuarios (200 OK)")
    void listar_DebeRetornarPaginaDeAuditorias() throws Exception {
        String token = obtenerTokenAdmin();

        mockMvc.perform(get("/api/auditoria/usuarios")
    ...
    @Test
    @DisplayName("Debe listar auditorías por ID de usuario (200 OK)")
    void listarPorUsuario_DebeRetornarPagina() throws Exception {
        String token = obtenerTokenAdmin();

        mockMvc.perform(get("/api/auditoria/usuarios/usuario/1")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", isA(java.util.List.class)));
    }
    ...
    void listarPorAccion_DebeRetornarPagina() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/usuarios/accion/CREATE")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", isA(java.util.List.class)));
    }
    ...
    void listarPorRangoFechas_DebeRetornarPagina() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/usuarios/fechas")
                        .param("inicio", inicio)
                        .param("fin", fin)
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
    ...
    void getResumenAuditorias_DebeRetornarMapaConConteos() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/usuarios/resumen")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
