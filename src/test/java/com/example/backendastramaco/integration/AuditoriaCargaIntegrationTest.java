...
class AuditoriaCargaIntegrationTest extends AuditoriaCargaBaseIntegrationTest {
    ...
    @Test
    @DisplayName("Debe listar todas las auditorías de cargas (200 OK)")
    void listar_DebeRetornarPaginaDeAuditorias() throws Exception {
        String token = obtenerTokenAdmin();

        mockMvc.perform(get("/api/auditoria/cargas")
    ...
    @Test
    @DisplayName("Debe listar auditorías por ID de carga (200 OK)")
    void listarPorCarga_DebeRetornarPagina() throws Exception {
        String token = obtenerTokenAdmin();

        mockMvc.perform(get("/api/auditoria/cargas/carga/1")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", isA(java.util.List.class)));
    }
    ...
    void listarPorTransportista_DebeRetornarPagina() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/cargas/transportista/1")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", isA(java.util.List.class)));
    }
    ...
    void listarPorAccion_DebeRetornarPagina() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/cargas/accion/CREATE")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", isA(java.util.List.class)));
    }
    ...
    void listarPorRangoFechas_DebeRetornarPagina() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/cargas/fechas")
                        .param("inicio", inicio)
                        .param("fin", fin)
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
    ...
    void getResumenAuditorias_DebeRetornarMapaConConteos() throws Exception {
        ...
        mockMvc.perform(get("/api/auditoria/cargas/resumen")
                        .header(AUTHORIZATION, BEARER_PREFIX + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.CREATE").exists())
                .andExpect(jsonPath("$.UPDATE").exists())
                .andExpect(jsonPath("$.UPDATE_AUMENTO").exists())
